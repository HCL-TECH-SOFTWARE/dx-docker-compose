...
docker exec dx-core sh -c "/opt/HCL/wp_profile/ConfigEngine/./ConfigEngine.sh enable-media-library -DWasPassword=wpsadmin -DPortalAdminPwd=wpsadmin -DdigitalAssets.baseUrl=http://$DX_HOSTNAME:3000 -DdigitalAssets.uiSuffix=/#/home/media?hcldam=true -Dexperience.api.url=http://$DX_HOSTNAME/dx/api/core/v1 -Dstatic.ui.url=http://$DX_HOSTNAME/dx/ui/dam/static"
docker exec dx-core sh -c "/opt/HCL/wp_profile/ConfigEngine/./ConfigEngine.sh enable-content-sites -DWasPassword=wpsadmin -DPortalAdminPwd=wpsadmin -Dcontentsites.static.ui.url=http://$DX_HOSTNAME/dx/ui/site-manager/static"
echo ""
echo "############################################################################"
