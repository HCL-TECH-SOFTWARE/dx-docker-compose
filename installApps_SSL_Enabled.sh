...
echo "#############################################################################"
echo "Installing CC, DAM and PeopleService portlets using DX_HOSTNAME=$DX_HOSTNAME"
echo "#############################################################################"
echo ""
docker exec dx-core sh -c "/opt/HCL/wp_profile/ConfigEngine/./ConfigEngine.sh enable-headless-content -DWasPassword=wpsadmin -DPortalAdminPwd=wpsadmin -Dstatic.ui.url=https://$DX_HOSTNAME/dx/ui/content/static"
docker exec dx-core sh -c "/opt/HCL/wp_profile/ConfigEngine/./ConfigEngine.sh enable-media-library -DWasPassword=wpsadmin -DPortalAdminPwd=wpsadmin -DdigitalAssets.baseUrl=https://$DX_HOSTNAME -DdigitalAssets.uiSuffix=/#/home/media?hcldam=true -Dexperience.api.url=https://$DX_HOSTNAME/dx/api/core/v1 -Dstatic.ui.url=https://$DX_HOSTNAME/dx/ui/dam/static"
